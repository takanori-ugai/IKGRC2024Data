...
//        explicitNulls = false
    ...
    for (scene in 1..7) {
        for (day in 1..10) {
            val f = File("Data/Answers/q1/q1_answer_scene${scene}_Day$day.json")
            val q1Answer = format.decodeFromString<Q1Answer>(f.readText(Charsets.UTF_8))
            println(q1Answer)
            println(Date().time)
            q1Answer.answers.forEach {
                val name = it.name.replace(Regex("[0-9]*_[a-z0-9]*"), "")
                val question =
                    Q1Qestion(
                        Date().time,
                        q1Answer.senario,
                        "How many times did he enter the $name?",
                        listOf(
                            Answer1(1, it.number, true),
                            Answer1(2, it.number + 1, false),
                        ),
                    )
                val fout = File("Data/QA/Q1/q1_answer_scene${scene}_Day${day}_$name.json")
                fout.writeText(format.encodeToString(question))
                val isEntered = Random.nextBoolean()
                val q2 = Q3Qestion(
                    Date().time,
                    q1Answer.senario,
                    "Did he enter the ${name} ${if (isEntered) it.number else it.number + 1} times?",
                    listOf(
                ...
                val f2 = File("Data/YesNo/Q1/q1_answer_scene${scene}_Day${day}_$name.json")
