...
// use the RDFDataMgr to find the input file
    val inputStream: InputStream = RDFDataMgr.open("file:Data/Admire_art1_scene1.ttl")
...
INSERT DATA {
    """
            ...
            if (place != null) {
                ...
                insertQuery.append("<${it["event"]}> vh2kg:place <http://kgrc4si.home.kg/virtualhome2kg/instance/Place$number> .\n")
            }
            val mainObject = it["mainObject"]
            if (mainObject != null) {
                deleteQuery.append("<${it["event"]}> :mainObject <$mainObject> .\n")
                insertQuery.append("<${it["event"]}> :mainObject <http://kgrc4si.home.kg/virtualhome2kg/instance/MainObject$number> .\n")
            }
            val targetObject = it["targetObject"]
            if (targetObject != null) {
                deleteQuery.append("<${it["event"]}> :targetObject <$targetObject> .\n")
                insertQuery.append("<${it["event"]}> :targetObject <http://kgrc4si.home.kg/virtualhome2kg/instance/TargetObject$number> .\n")
            }
            deleteQuery.append("<${it["event"]}> :action <${it["action"]}> .\n")
            deleteQuery.append("}\n")
            insertQuery.append("<${it["event"]}> :action <http://kgrc4si.home.kg/virtualhome2kg/ontology/action/Action$number> .\n")
            insertQuery.append("}\n")
            println(it["place"])
